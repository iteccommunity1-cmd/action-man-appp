...
interface TimeEntry {
  ...
  description?: string;
  created_at: string;
  tasks?: Pick<Task, 'id' | 'title'>; // Only select necessary task fields
}
...
export const TimeEntryList: React.FC<TimeEntryListProps> = ({ projectId }) => {
  ...
  const { data: timeEntries, isLoading, isError, error } = useQuery<TimeEntry[], Error>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('time_entries')
        .select('*, tasks(id, title)') // Fetch associated task title
        .eq('project_id', projectId)
        .eq('user_id', currentUser?.id) // Only show current user's entries
  ...
  if (isError) {
    showError("Failed to load time entries: " + error.message);
    return (
      <div className="text-center p-8 text-destructive">
        <p>Error loading time entries. Please try again later.</p>
      </div>
    );
  }
            ...
            <Card key={entry.id} className="rounded-xl glass-card">
              <CardContent className="p-4 flex items-start justify-between space-x-4">
                ...
                <div className="flex space-x-2 flex-shrink-0">
                  ...
                  {/* Re-added delete button, visible only if current user created the entry */}
