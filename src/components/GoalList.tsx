...
export const GoalList: React.FC<GoalListProps> = ({ projectId }) => {
  ...
  const getStatusBadgeColor = (status: Goal['status'], dueDate?: string) => {
    ...
    // Corrected: Removed redundant 'status !== 'achieved'' check
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent className="rounded-xl p-6 bg-card border border-border text-card-foreground glass-card">
          ...
          <AlertDialogFooter className="pt-4">
            ...
            <AlertDialogAction onClick={confirmDeleteGoal} className="rounded-lg bg-destructive hover:bg-destructive/90 text-destructive-foreground px-4 py-2">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
