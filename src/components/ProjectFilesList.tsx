...
export const ProjectFilesList: React.FC<ProjectFilesListProps> = ({ projectId }) => {
  ...
  const confirmDeleteFile = async () => {
    ...
    try {
      ...
      if (storageError) {
        console.warn("[ProjectFilesList] Error deleting file from storage:", storageError);
      }
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent className="rounded-xl p-6 bg-card border border-border text-card-foreground glass-card">
          ...
          <AlertDialogFooter className="pt-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
            ...
            <AlertDialogAction onClick={confirmDeleteFile} className="rounded-lg bg-destructive hover:bg-destructive/90 text-destructive-foreground px-4 py-2 w-full sm:w-auto">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
